import { ACCOUNT, CHANGE_PLAN, MEMBER_TERM, PLAN_DETAILS, SETTINGS, SIGN_OUT_BUTTON } from "@/constants";
		...
		<div>
			...
			<main className="mx-auto max-w-6xl px-5 pt-24 pb-12 transition-all md:px-10">
				<div className="flex flex-col gap-x-4 md:flex-row md:items-center">
					...
					<div className="-ml-0.5 flex items-center gap-x-1.5">
						...
						<p className="text-xs font-semibold text-[#555]">
							{MEMBER_TERM} {subscription?.created}
						</p>
					</div>
				</div>
				...
				<div className="mt-6 grid grid-cols-1 gap-x-4 border px-4 py-4 md:grid-cols-4 md:border-x-0 md:border-t md:border-b-0 md:px-0 md:pb-0">
					<h4 className="text-lg text-[gray]">{PLAN_DETAILS}</h4>
					<div className="col-span-2 font-medium">
						{products && products.filter((product) => product.id === subscription?.product)[0]?.name}
					</div>
					<p className="cursor-pointer text-blue-500 hover:underline md:text-right">{CHANGE_PLAN}</p>
				</div>
